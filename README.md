...
- **One-push cloud deploy:** [`tag-go/deploy/`](tag-go/deploy/README.md) packages the gateway as a container — a multi-stage `Dockerfile` (CGO-free static binary, non-root Alpine runtime, `/health` HEALTHCHECK), a repo-root [`render.yaml`](render.yaml) Blueprint (auto-generated `TAG_GATEWAY_KEY`), and an entrypoint that refuses a public bind without a key unless `TAG_ALLOW_UNAUTHENTICATED=1` — with recipes for Docker, Render, Hugging Face Spaces (port 7860), and UptimeRobot keep-warm.
