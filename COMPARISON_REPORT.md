...
**Remaining Python-only commands** are the intentional managed-Hermes runtime-passthrough layer (`chat`, `gateway`, `kanban`, `runtime`, `sessions`, `status`, `dashboard`, `config`, `profile`, `submit`, `update`, `skills`, `tools`) plus `desktop` (OS packaging) — deliberate non-ports in the "Go owns its own runtime" design, where `serve`/`devui`/`web` replace the dashboard and the Go binary doesn't ship the Python Hermes checkout.
