...
- [x] **sandbox run --backend docker** — `sandbox run` now takes `--backend {restricted,docker}` (default `restricted`, so nothing regresses). The docker backend (`ExecDocker` + a pure, deterministic `dockerArgs` builder) shells out to `docker run --rm --memory <m> --cpus <c> --network <n> [--workdir <d>] <image> sh -c <cmd>` with hardened defaults (`--memory 512m`, `--cpus 1`, `--network none`; overridable via `--memory/--cpus/--network`). `--image` is required for the docker backend; when docker is absent from PATH it errors clearly (`docker not found on PATH: install Docker or use --backend restricted`). A timeout force-removes the named container and yields `Exit=124` / `TimedOut=true`, reusing the existing `{stdout,stderr,exit,timed_out}` `Result` shape. `--json` unchanged. (`internal/sandbox/docker.go`, `cli/sandbox.go`)
- E2E: unit tests for `dockerArgs` (asserts `--rm`/`--memory 512m`/`--cpus 1`/`--network none`/`--workdir`/`<image> sh -c` tail, no docker invoked) + input validation (empty command, missing image, non-positive timeout); real-container E2E (echo/nonzero/network-none/timeout) is written but skips when the docker daemon is unreachable (`docker info` probe). — impl + E2E done on branch, merge pending.
- Limitation: only `restricted` (default) and `docker` backends exist; `modal`/`gpu`/`gui` backends remain future work.
