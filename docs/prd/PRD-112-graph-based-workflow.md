...
**Status:** Partial — conditional edges and state reducers now ship, added to the SPEC-driven `dag` engine that exists rather than as a second `tag workflow graph` engine over compiled-in Go workflows (that surface is left free for PRD-109's interrupt, and `tag plan decompose`/PRD-105 is untouched). Three new optional step keys: `when` (a conditional edge — `{source,op,value}` over ops equals/not_equals/contains/not_contains/prefix/suffix/matches/empty/non_empty, resolved and validated at save time), `output` (the state key the node publishes under; defaults to the step name or `step<N>`), and `reduce` (last/first/append/concat/sum/merge; steps sharing an output key must agree on the reducer). A node whose guard is false goes terminal-`skipped` and everything behind it is cascade-skipped, so the untaken branch genuinely does not run; `{{state.<key>}}` in a task is interpolated from the reduced state at dispatch time, and an unresolvable reference fails the job rather than substituting "". `tag dag state [RUN_ID]` shows the per-run reduced state, each node's status and the reason a node was skipped. Every pre-existing property is intact (dependency-ordered waves, per-job `paths.WorkDir`, SIGTERM via `signal.NotifyContext`, integer dependency-index validation); `Drain` additionally no longer stops before a cascade chain finishes resolving. NOT implemented: cyclic/retry edges with `maxIterations`, subgraph nodes, checkpoint/resume of a graph session, `workflow_sessions`/`workflow_steps` tables, and mermaid `describe` output
**Priority:** P1
