...
**Status:** Shipped (Go) — extraction is a real two-stage pipeline (`internal/memory/extract.go`) and `tag run` has an **opt-in** post-run trigger. `tag run --auto-memorize` / `--no-auto-memorize` force it per invocation; otherwise it follows `memory.auto_extract` (per-profile, then global, then `TAG_MEMORY_AUTO_EXTRACT`), managed by `tag mem2 config show|set`. Default is OFF, because extraction costs an LLM call. Every extracted memory is written with `source='auto_extract'`; candidates are MD5-deduped against the store, secret-scanned before write (AC-05), and screened for prompt-injection phrasing. Runs are audited in `memory_extraction_runs` (created by the idempotent `EnsureExtractSchema`, not `schema.sql`) and surfaced by `tag mem2 extractions`; a 10/hour-per-profile rate limit fires *before* the LLM call. `mem2 extract` gains `--provider`, `--model`, `--dry-run`, `--max-turns`. **Offline-honest:** phase 1 requires a strict JSON fact array, so `--provider echo` extracts nothing and says why rather than fabricating. **Never fails the parent run:** the hook is a deferred call after the run's output, all failures become stderr warnings, and stdout stays a single parseable `--json` object. Deferred vs. the PRD text: the phase-2 UPDATE_MEMORY LLM reconciliation pass (ADD/UPDATE/DELETE/NOOP with supersession) — only exact-duplicate NOOP is implemented — plus cosine-threshold semantic dedup, `extraction_logs`, cost accounting, and per-profile custom prompt files.
**Priority:** P1
