...
**Update 2026-08-21.** Three of those gaps are now closed. The `interrupt` action and a `require-approval` rule type ship in the engine (block > interrupt > warn precedence; fail-closed never downgrades to an approvable pause) — and PRD-109's HITL primitive (`internal/hitl`) **does** exist, so the interrupt action is not blocked (the live run-loop wiring is tracked separately). The §6 surface ships as `tag guardrail runtime` (a byte-for-byte alias over the `tag tripwire` engine). And §6/G7 `add`/`remove` ship as **validated `tag.yaml` editors** (effective next run), deliberately **superseding** the §9.1 `runtime_guardrail_configs` DDL: a hot-mutable table read once at start would either be inert until restart or violate NFR-03's immutable-ruleset doctrine, so §9.1 is treated as superseded by the config-driven approach rather than built. The stdlib `path.Match` tool-glob (in place of the §NFR-04 `gobwas/glob`) is likewise intended, avoiding a new dependency the tree does not need.
**Priority:** P1
