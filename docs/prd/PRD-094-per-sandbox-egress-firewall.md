...
**Status:** Partial — per-sandbox egress rules now ship, and what each backend does is stated exactly rather than uniformly. `sandbox run` takes `--egress open|restricted|pypi`, `--allow-host/--deny-host/--allow-cidr/--deny-cidr`, `--deny-all/--allow-all`; `sandbox firewall list/show/test` inspects a policy offline. **docker backend: genuinely ENFORCED per destination.** The policy becomes kernel routes (a blackhole per denied range, an on-link or via-gateway route per allowed range, a blackhole default under default-deny) installed in a network namespace owned by a separate `--cap-add NET_ADMIN` helper container; the workload joins it with `--network container:<helper>` and holds no capabilities, so it cannot alter the rules. Verified against live Docker: an allowed target reachable, a sibling target blocked, both reachable in the control run. The connected subnet and `fe80::/64` are blackholed too — a blackhole default alone left every sibling container on the docker network reachable, which the integration test caught — and the docker gateway is re-added only when an allowed destination needs it as a next hop. Hostname rules are pinned to the addresses they resolved to at policy time and injected as `/etc/hosts` with no working resolver; the TOCTOU window, the absence of port granularity, and the absence of per-connection violation logging are all disclosed in `Result.Isolation`. A blanket denial needs no helper (`--network none` is the enforcement). Every failure is fail-closed exit 127: helper won't start, policy won't install, a wildcard or unresolvable hostname rule, an IPv6 allow with no v6 gateway, or `--network none` contradicting an allow list. **restricted backend, every OS: per-destination rules are REFUSED with exit 127**, naming the platform reason (macOS SBPL is `(deny network*)`, all-or-nothing; Linux offers a namespace that removes every interface and Landlock ABI≥4 that denies all TCP) and pointing at `--backend docker` — never silently ignored. A blanket `--deny-all` IS accepted there and is then *required*: the weaker no-namespace Linux fallback is removed from the chain so the run fails rather than degrading to TCP-only. NOT implemented: stored per-profile policies (`firewall add/remove`), `sandbox_firewall_violations` + the JSONL audit log + violation spans (a blackhole route drops packets with no counter TAG can attribute to a rule), `network:` in profile YAML, wildcard-hostname rules, and IPv6 allow rules
**Priority:** P3
